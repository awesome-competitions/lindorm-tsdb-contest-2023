...
import com.alibaba.lindorm.contest.util.Deflate;
import com.alibaba.lindorm.contest.util.XZ;
...
//        }
        ...
        for (int i = 0; i < strings.length; i ++){
            ...
            ByteBuffer srcBuffer = ByteBuffer.allocateDirect(input.length);
            ByteBuffer encodeBuffer = ByteBuffer.allocateDirect(1000000);
            ByteBuffer decodeBuffer = ByteBuffer.allocateDirect(1000000);

            srcBuffer.put(input);
            srcBuffer.flip();
            XZ.compress(encodeBuffer, srcBuffer);
            encodeBuffer.flip();
            int compressed = encodeBuffer.remaining();

            XZ.decompress(decodeBuffer, encodeBuffer);
            decodeBuffer.flip();
