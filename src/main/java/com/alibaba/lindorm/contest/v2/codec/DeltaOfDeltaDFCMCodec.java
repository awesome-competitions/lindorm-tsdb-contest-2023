...
public class DeltaOfDeltaDFCMCodec extends Codec<double[]>{
    @Override
    public void encode(ByteBuffer src, double[] data) {
        ...
        if (data.length > 1){
            ...
            for (int i = 1; i < data.length; i++) {
                ...
                if (xorValue == 0){
                    buffer.putBit(false);
                }else if (preTrailingZeros >= 0 && trailingZeros >= preTrailingZeros) {
                    buffer.putBit(true);
                    buffer.putBit(true);
    ...
    public static void main(String[] args) {
        DeltaOfDeltaDFCMCodec varintCodec = new DeltaOfDeltaDFCMCodec();
